...
Use **Python 3.12 or 3.13**. The `pandas-ta` version in `requirements.txt`
requires Python 3.12 or newer; the accompanying Numba pin is not intended for
Python 3.14. Python 3.11 can compile the scripts but cannot install this
dependency set.

Clone the repository and create an isolated environment:

```bash
git clone https://github.com/agraolmez/ml-spy-trading.git
cd ml-spy-trading
...
`.venv\Scripts\Activate.ps1` in PowerShell. Python 3.12 may be used in place of
3.13. If LightGBM reports a missing `libomp.dylib` on macOS, install its OpenMP
runtime with `brew install libomp`, then retry the import.

Run all commands from the repository root so relative data paths resolve.
Keep the checked-in CSV and Excel results unchanged if you want to retain the
published research snapshot. Use a separate clone for reruns: the scripts write
result files in the working directory and may replace existing outputs.
...
This loads `spy_ohlcv_macro_data_2000_2025.csv` when present, runs the selected
model's walk-forward comparison, prints metrics, and displays a plot. If the
CSV is missing, the script attempts to download market data instead. Keep the
bundled CSV for comparison with the published historical data snapshot.

### Run the full research search

```bash
python final.py
```

This is the longer experiment: a 5,000-trial model search followed by
walk-forward and bootstrap testing. It downloads market data and writes
`model_results_seed42.xlsx`, `walk_forward_backtest_results.xlsx`, and
`final_combined_backtest_results.xlsx`. It is not run automatically by CI.

### What the automated checks cover

GitHub Actions checks dependency installation, dependency consistency, Python
syntax, and a small synthetic-data indicator/model smoke test on Python 3.12
and 3.13. Passing checks do **not** establish that the full market-data
experiment has been rerun or that the reported performance has been reproduced.
This is a research repository, not a live trading service.
